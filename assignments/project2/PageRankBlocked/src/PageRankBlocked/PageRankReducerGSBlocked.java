...
public class PageRankReducerGSBlocked extends Reducer<Text, Text, Text, Text> {
	...
	protected void reduce(Text key, Iterable<Text> values, Context context)
		...
		while (Double.compare(residual, CONST_DOUBLE_THRESHOLD) > 0) {
			...
			++counter;
			residual = (double) (residual / (double) initialPassPRMap
					.size());
		}
		...
		context.getCounter(PAGE_RANK_COUNTER.NODE_COUNT).increment((long)nodeCount);
		
		Double residualBlock = (double) (residual/(double) nodeCount);
		Long residualLong = (long) (residual * 10000);
		context.getCounter(PAGE_RANK_COUNTER.RESIDUAL).increment((long)residualLong);
